...
# https://www.kaggle.com/oysteijo/ghouls-goblins-and-ghosts-boo/ghosts-n-goblins-n-neural-networks-lb-0-74858/comments
...
y_train = pd.get_dummies(train[["type"]], prefix="")  # 目标值用了one hot 编码
...
## A dead simple neural network class in Python+Numpy. Plain SGD（随机梯度下降）, and no regularization.
def sigmoid(X):
    return 1.0 / ( 1.0 + np.exp(-X) )

def softmax(X):
    _sum = np.exp(X).sum()
    return np.exp(X) / _sum  #是K维向量，K是类别总数
...
class neuralnet(object):
    ...
    def forward(self,X):
        net1 = np.matmul( X, self._W1 ) + self._b1   # 第一层隐含层的激活
        y = sigmoid(net1)
