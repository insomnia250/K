...
# data_train.loc[(data_train.NameTitle!='Mr')&(data_train.NameTitle!='Mrs')\
...
# data_test.loc[(data_test.NameTitle!='Mr')&(data_test.NameTitle!='Mrs')\
...
'''-----------------------预测-----------------------'''
Prediction = DataFrame({'Survived':bagging_clf.predict(test_set_np).astype(int)})


Result = pd.concat([data_test['PassengerId'] , Prediction] , axis = 1)

print 'total survived' , Result.Survived.sum()
Result.to_csv('../bagging_prediction.csv' , index = False)